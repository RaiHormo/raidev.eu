...
fetch('/gallery/art-database.json')
    ...
    .then(data => {
        ...
        for (var i = 0; i< database.artwork.length; i++) {
            ...
            img.setAttribute('alt', picdata.title + " - " + date.getFullYear() + " " + date.toLocaleString('en-us', { month: 'short' }) + " "+ date.getDate());
...
function setSpan(img) {
    ...
    img.style.gridRowEnd = "span "+Math.round((img.getBoundingClientRect().height+rowGap)/(rowHeight+rowGap));
}
